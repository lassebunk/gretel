...
Then, in <code>config/initializers/breadcrumbs.rb</code>:
...
At the top of <code>app/views/issues/show.html.erb</code>:
...
This will generate a <code><div></code> containing the breadcrumbs.
...
Options for <code><%= breadcrumb %></code>:
  ...
  :posttext         Text to be appended after breadrcumb, if any: Default: none
  ...
  :semantic         Whether it should generate semantic breadcrumbs as per:
                    http://support.google.com/webmasters/bin/answer.py?hl=en&answer=185417
                    Default: false
  :id               HTML element ID to be inserted. Default: none
