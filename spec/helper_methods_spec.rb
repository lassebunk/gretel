...
class HelperMethodsTest < ActionView::TestCase
  ...
  it("no breadcrumb") { assert_dom_equal("", breadcrumbs.to_s) }
  ...
  it("parent breadcrumb yields only if present") do
    breadcrumb(:basic)
    out = parent_breadcrumb(:autoroot => false) { "yielded" }
  ...
  it("passing options to links") do
    ...
    breadcrumbs(:autoroot => false).tap do |links|
      links[0].tap do |link|
        ...
        expect((not link.nonexistent?)).to(be_truthy)
  ...
  # File change interval is 1 second
